...
{
	if(GPIOPinRead(GPIO_PORTN_BASE, 0x2))
		return 1;
...
{
	if(GPIOPinRead(GPIO_PORTN_BASE, 0x1))
		return 1;
...
{
	if(GPIOPinRead(GPIO_PORTF_BASE, 0x10))
		return 1;
...
{
	uint8_t duty_cycle_pct = (keyboard_buffer[0] - '0') * 10 + (keyboard_buffer[1] - '0') * 1;
	set_duty_cicle(duty_cycle_pct);
}
